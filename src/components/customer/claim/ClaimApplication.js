...
class ClaimProcess extends Component {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            claim: {
                ...
                type: '',
                date: '',
                insurance: this.props.currentInsurance
            },
        }
    ...
    handleFileUpload = (event) => {
        ...
        this.setState(state => ({
            claim: {...state.claim, files: newFiles}
        }));
    };
    ...
    handleForm = event => {
        this.handleNext();
        this.props.postClaim(this.state.claim);
        event.preventDefault();
        setTimeout(() => this.props.history.push(BaseUrl.claimsPath), 3000)
    }

    getFilesName = () => {
        let items = [];
        Array.from(this.state.claim.files).forEach(file => {
    ...
    render() {
            ...
            return (
                <form id="claims_form" onSubmit={this.handleForm}>
                    <Grid container spacing={24} className={classes.details}>
                        <Grid item xs={12} md={6}>
                            <FormControl margin="normal" required fullWidth>
                                ...
                                <Input
                                    ...
                                    variant="outlined"
                                    value={this.state.claim.location}
                        ...
                        <Grid item xs={6}>
                            <FormControl margin="normal" required fullWidth>
                                <InputLabel htmlFor="type" shrink={true}>Type</InputLabel>
                                <Input
                                    required
                                    type="type"
                                    id="type"
                        ...
                        <Grid item xs={12}>
                            ...
                            {this.state.claim.files.length !== 0 &&
                                this.getFilesName()
                            }
                        </Grid>
                    </Grid>
        ...
        const accidentType = (tiers) => {
            return (tiers.map(tier => (
                <Grid item key={tier.type} xs={12} md={4}>
                    <Card>
                        <CardHeader
                                    ...
                                    {activeStep === 1 && (
                                        <div className={classes.smallContainer}>
                                            <Paper className={classes.paper}>
                                                ...
                                                <Typography variant="body1" gutterBottom>
                                                    Filling Details {JSON.stringify(this.state.claim)}
                                                </Typography>
                                                {filingForm()}
                                            </Paper>
